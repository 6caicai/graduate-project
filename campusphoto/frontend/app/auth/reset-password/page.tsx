...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await CampusPhotoApi.resetPassword(token, formData.password)
      setIsSuccess(true)
      toast.success('密码重置成功！')
    } catch (error: any) {
      toast.error(error.response?.data?.detail || '重置失败，请重试')
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        >
          <Link
            href="/auth/login"
            className="text-sm font-medium text-pink-600 hover:text-pink-500 dark:text-pink-400 dark:hover:text-pink-300 transition-colors"
          >
            返回登录页面
          </Link>
        </motion.div>
